...
// Function to generate server.js file
function generateServerJs(config: any): string {
  const serverTemplate = `// MCP Server generated by buildmcp.space
const { createServer } = require('@modelcontextprotocol/sdk');

const resources = ${JSON.stringify(config.resources || [], null, 2)};
const tools = ${JSON.stringify(config.tools || [], null, 2)};
const prompts = ${JSON.stringify(config.prompts || [], null, 2)};
...
- \`mcp_server.js\`: The main server file
      ...
      if (supabaseAdmin) {
        ...
        if (deploymentError || !deploymentData) {
          ...
          await storeDownloadData(id, {
            ...
            serverFiles: {
              'mcp_server.js': generateServerJs(mcpData.config),
